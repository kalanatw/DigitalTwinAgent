...
from .models import (
    ...
    CSVDocument, CSVDataset, CSVColumn
)
    ...
    def process_csv_file(self, document):
        ...
        try:
            ...
            result = self.csv_processor.process_csv_file(
                ...
                twin_version_id=str(document.twin_version.id) if document.twin_version else None,
                user_id=str(document.uploaded_by.id) if document.uploaded_by else None
            )
            
            if not result.get("success", False):
                document.status = "failed"
                document.processing_error = result.get("error", "Unknown error processing CSV file")
                document.save()
                logger.error(f"Error processing CSV file {document.title}: {document.processing_error}")
                return False
            ...
            logger.info(f"Created {len(stored_chunks)} chunks for document {document.title}")
...
class CSVProcessor:
    """Class to handle CSV file processing and intelligent analysis."""
    
    def __init__(self):
        self.sample_size = 1000  # Maximum rows to sample for schema inference
    ...
    def process_csv_file(self, file_path: str, filename: str, twin_version_id: str = None, user_id: str = None) -> Dict[str, Any]:
        """
        ...
        csv_doc = self._save_to_database(
            df, filename, schema, time_series, 
            twin_version_id, user_id, file_path
        )
            ...
            for encoding in encodings:
                for sep in separators:
                    try:
                        ...
                        logger.info(f"Successfully loaded CSV with encoding {encoding} and separator {sep}")
                        return df
                    except Exception:
                        continue
    ...
    def _analyze_column(self, df: pd.DataFrame, column: str) -> Dict[str, Any]:
        ...
        if data_type in ["integer", "float", "numeric"]:
            ...
            try:
                ...
                col_info["statistics"].update({
                    "min": datetime_data.min().isoformat() if not pd.isna(datetime_data.min()) else None,
                    "max": datetime_data.max().isoformat() if not pd.isna(datetime_data.max()) else None,
                    "range_days": (datetime_data.max() - datetime_data.min()).days if not pd.isna(datetime_data.min()) and not pd.isna(datetime_data.max()) else None
                })
            ...
            if unique_count / len(df) < 0.5:  # If less than 50% unique values
                col_info["statistics"]["frequent_values"] = {
                    str(k): int(v) for k, v in col_data.value_counts().head(10).to_dict().items()
                }
    ...
    def _detect_data_type(self, series: pd.Series, column_name: str) -> Tuple[str, List[str]]:
        """
        ...
        try:
            ...
            if np.all(numeric_series.astype(int) == numeric_series):
                ...
                if numeric_series.nunique() < 15 and numeric_series.nunique() / len(numeric_series) < 0.05:
                ...
                # Check if it's money
                elif any(curr in column_name.lower() for curr in ["price", "cost", "amount", "salary"]):
    ...
    def _is_datetime(self, series: pd.Series, column_name: str) -> bool:
        ...
        date_patterns = ["date", "time", "dt", "day", "month", "year", "created", "updated"]
        name_suggests_date = any(pattern in column_name.lower() for pattern in date_patterns)
    ...
    def _analyze_relationships(self, df: pd.DataFrame, columns_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for col_name, col_info in columns_info.items():
            ...
            if (col_name.lower().endswith("_id") and 
                col_info["data_type"] == "integer" and
                "unique" not in col_info["tags"]):
                
                # Look for potential primary key references
                for ref_col, ref_info in columns_info.items():
                    if ("unique" in ref_info["tags"] and 
                        ref_info["data_type"] == "integer" and
                        col_name != ref_col):
                        
                        # Check if values in col_name are subset of ref_col
                        try:
                            overlap = set(df[col_name].dropna()) <= set(df[ref_col].dropna())
                            ...
                            if overlap:
                                relationships.append({
                                    ...
                                    "to_column": ref_col,
                                    "confidence": "high" if "primary_key" in ref_info["tags"] else "medium"
                                })
                        except:
                            pass
    ...
    def _identify_primary_key(self, df: pd.DataFrame, columns_info: Dict[str, Any]) -> Optional[str]:
        ...
        for col_name, col_info in columns_info.items():
            ...
            elif (col_info["data_type"] == "integer" and 
                  col_info["statistics"]["unique_percentage"] > 90):
    ...
    def _detect_time_series(self, df: pd.DataFrame, schema: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        temporal_columns = []
        date_column_patterns = ["date", "time", "timestamp", "datetime", "day", "month", "year"]
        ...
        for time_col in temporal_columns:
            ...
            numeric_columns = []
            for col_name, col_info in schema["columns"].items():
                if (col_info["data_type"] in ["integer", "float", "numeric"] and
                    col_name != time_col and
                    "identifier" not in col_info["tags"]):
            ...
            try:
                ...
                logger.warning(f"Error processing time series for column {time_col}: {str(e)}")
                continue
    ...
    def _save_to_database(self, 
                         ...
                         user_id: str = None,
                         file_path: str = None) -> CSVDocument:
    ...
    def _generate_stats_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if not numerical_cols.empty:
                summary["numerical"] = {
                    "mean": {col: float(val) for col, val in numerical_cols.mean().to_dict().items()},
                    "median": {col: float(val) for col, val in numerical_cols.median().to_dict().items()},
                    "std": {col: float(val) for col, val in numerical_cols.std().to_dict().items()},
                    "min": {col: float(val) for col, val in numerical_cols.min().to_dict().items()},
                    "max": {col: float(val) for col, val in numerical_cols.max().to_dict().items()}
                }
            ...
            summary["null_stats"] = {
                ...
                "null_percentage": float((sum(null_counts.values()) / (len(df) * len(df.columns))) * 100)
            }
